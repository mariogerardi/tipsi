...
function Frozen() {
    ...
    const getFrozen = async () => {
        try {
            const response1 = await fetch('https://www.thecocktaildb.com/api/json/v1/1/search.php?s=frozen');
            const json1 = await response1.json();
            setData1(json1);
            const response2 = await fetch('https://www.thecocktaildb.com/api/json/v1/1/search.php?s=frose');
            const json2 = await response2.json();
            setData2(json2);
            const response3 = await fetch('https://www.thecocktaildb.com/api/json/v1/1/search.php?s=cooler');
    ...
    return (
        <View style={styles.container}>
            <Text style={styles.header}>frozen friday</Text>
            {isLoading ? <ActivityIndicator /> : (
                <FlatList
                    ...
                    style={styles.list}
                    data={data2.drinks.concat(data1.drinks).concat(data3.drinks)}
                    keyExtractor={({ id }) => id}
