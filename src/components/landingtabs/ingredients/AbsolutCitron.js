...
function AbsolutCitron() {

    const [isLoading, setLoading] = useState(true);
    const [data, setData] = useState([]);

    const getAbsolutCitron = async () => {
        try {
            const response = await fetch('https://www.thecocktaildb.com/api/json/v1/1/filter.php?i=absolut_citron');
    ...
    return (
        <View style={styles.container}>
            <Text style={styles.header}>got absolut citron?</Text>
